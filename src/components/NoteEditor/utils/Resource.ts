...
export default class Resource extends BaseItem {
	...
	*/
	...
	static fetchStatusToLabel(status: number) {
		if (status === Resource.FETCH_STATUS_IDLE) return ('Not downloaded');
		if (status === Resource.FETCH_STATUS_STARTED) return ('Downloading');
		if (status === Resource.FETCH_STATUS_DONE) return ('Downloaded');
		if (status === Resource.FETCH_STATUS_ERROR) return ('Error');
